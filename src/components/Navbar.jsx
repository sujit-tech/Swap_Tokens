...
const NavbarItem = ({ title, classProps }) => {
  return <li className={`mx-4 cursor-pointer  ${classProps} shadow  hover:shadow-indigo-500/50`}>{title}</li>;
};

const Navbar = () => {
    const [toggleMenu,setToggleMenu] = React.useState(false);
  return (
    <nav className="w-full flex justify-start justify-around items-center p-4 ">
      ...
      <ul className="text-white md:flex hidden list-none flex-row justify-between items-center flex-initial">
        ...
        <li className="bg-indigo-500 shadow-lg shadow-indigo-500/50 relative bg-[#2952e3] py-2 px-7 mx-4 rounded-full cursor-pointer hover:mix-blend-luminosity">
          Login
        </li>
      </ul>
